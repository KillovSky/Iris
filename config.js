...
module.exports = kconfig = async (kill, message) => {
	...
	try {
		...
		if (!tools('cooldown').isFiltered(from) && !isMedia && !isCmd) {
			try {
				if (message.body.includes(`@${botNumber.replace('@c.us', '')}`)) {
					let irisLine = await tools('others').getRandLine(1, './lib/config/Utilidades/reply.txt')
					await kill.reply(from, irisLine, id)
				}
			} catch (error) {
				/*Nada a ser feito*/
			}
		...
		args.filter(c => c.includes('/')).map(b => cmdtorun.push(b.slice(1)))
		...
		switch (cmdtorun[i]) {
			...
			case 'gifsticker':
				...
				if (isMedia && isImage || isQuotedImage) {
					let mediaData = await decryptMedia(encryptMedia)
					var isCircle = arks.includes('-circle') ? false : true
			...
			case 'oculto':
				if (!isGroupMsg) return await kill.reply(from, mess.sogrupo(), id)
				await kill.sendTextWithMentions(from, mess.oculto(randomMember, await tools('others').getRandLine(1, './lib/config/Utilidades/porn.txt')))
			break
			...
			case 'life':
				if (region == 'en') return await kill.reply(from, await tools('others').getRandLine(1, './lib/config/Utilidades/fml.txt'), id)
				await translate(await tools('others').getRandLine(1, './lib/config/Utilidades/fml.txt'), {
					to: region
				}).then(async (lfts) => {
			...
			case 'iris':
				if (args.length == 0) return await kill.reply(from, await tools('others').getRandLine(1, './lib/config/Utilidades/reply.txt'), id)
				try {
					if (argl[0] == '-g') {
						...
						if (awnser == '') {
							await kill.reply(from, await tools('others').getRandLine(1, './lib/config/Utilidades/reply.txt'), id)
			...
			case 'speak':
				...
				if (args.length == 0) return await sppt.save(speakttplc, await tools('others').getRandLine(1, './lib/config/Utilidades/reply.txt'), async function() {
				...
				try {
					if (argl[0] == '-g') {
						...
						if (voiceaw == '') {
							await sppt.save(speakttplc, await tools('others').getRandLine(1, './lib/config/Utilidades/reply.txt'), async function() {
								await kill.sendPtt(from, speakttplc, id)
							})
			...
			case 'curiosidade':
				try {
					if (argl[0] == '-g') {
						let curist = await shell.exec(`bash -c "cd lib/config/Utilidades && grep -i "${args[1]}" curiosidades.txt | shuf -n 1"`, {
							silent: true
						}).stdout
						if (curist == '') {
							await kill.reply(from, await tools('others').getRandLine(1, './lib/config/Utilidades/curiosidades.txt'), id)
						} else return await kill.reply(from, curist, id)
					} else return await kill.reply(from, await tools('others').getRandLine(1, './lib/config/Utilidades/curiosidades.txt'), id)
				} catch (error) {
					await kill.reply(from, await tools('others').getRandLine(1, './lib/config/Utilidades/curiosidades.txt'), id)
			...
			case 'trecho':
				try {
					if (argl[0] == '-g') {
						let trecd = await shell.exec(`bash -c "cd lib/config/Utilidades && grep -i "${args[1]}" frases.txt | shuf -n 1"`, {
							silent: true
						}).stdout
						if (trecd == '') {
							await kill.reply(from, await tools('others').getRandLine(1, './lib/config/Utilidades/frases.txt'), id)
						} else return await kill.reply(from, trecd, id)
					} else return await kill.reply(from, await tools('others').getRandLine(1, './lib/config/Utilidades/frases.txt'), id)
				} catch (error) {
					await kill.reply(from, await tools('others').getRandLine(1, './lib/config/Utilidades/frases.txt'), id)
					await kill.reply(from, mess.fail(command, error, time), id)
				}
			...
			case 'perfil':
				if (isGroupMsg) {
					...
					var namae = quotedMsg ? quotedMsgObj.sender.pushname : (mentionedJidList.length !== 0 ? await kill.getContact(mentionedJidList[0]).pushname : pushname)
					...
					try {
						...
						await translate(await tools('others').getRandLine(1, './lib/config/Utilidades/biblia.txt'), {
							to: region
						}).then((bibles) => {
						...
						await translate(await tools('others').getRandLine(1, './lib/config/Utilidades/cantadas.txt'), {
							to: region
						}).then((love) => {
							customTexts.getGirlfriend = love.text
						})
					} catch (err) {
						customTexts.fuckALLife = tools('others').getRandLine(1, './lib/config/Utilidades/fml.txt')
						customTexts.GodKillsToo = tools('others').getRandLine(1, './lib/config/Utilidades/biblia.txt')
						customTexts.getGirlfriend = tools('others').getRandLine(1, './lib/config/Utilidades/cantadas.txt')
					}
			...
			case 'brainly':
				if (args.length == 0 || args.length >= 10) return await kill.reply(from, mess.tenargs(), id)
				let brain = new brainly(region)
				await brain.search(region, body.slice(9), 1).then(async (res) => {
			...
			case 'get':
				if (isQuotedImage || isQuotedAudio || isQuotedPtt || isQuotedVideo || isQuotedSticker || isQuotedGif || quotedMsgObj || isMedia) {
					let mediaData = await decryptMedia(encryptMedia)
					await kill.sendFile(from, tools('others').dataURI(quotedMsgObj.mimetype, mediaData), '', '', id)
			...
			case 'maps':
				if (args.length == 0) return await kill.reply(from, mess.noargs() + 'city names/nomes de cidade/nombres de ciudad.', id)
				// VNHipIZfFbGtfPl14QXy // eEvtExNmE8dAgYsvPIoR
			...
			case 'corno':
				corno = quotedMsg ? quotedMsgObj.sender.id : (mentionedJidList.length !== 0 ? mentionedJidList[0] : sender.id)
				await kill.sendTextWithMentions(from, `@${corno.replace('@c.us')} é ${lvpc}%...\n\n"${tools('others').getRandLine(1, './lib/config/Utilidades/corno.txt')}"\n\nE nas horas vagas ${lvrq}%...\n\n"${tools('others').getRandLine(1, './lib/config/Utilidades/corno.txt')}" 😳.`)
			break
			...
			case 'lgbt':
				...
				await canvacord.Canvas.rainbow(theLgBtic).then(async (buffer) => {
					await kill.sendFile(from, tools('others').dataURI('image/png', buffer), `gay.png`, mess.lgbt(lvpc, await tools('others').getRandLine(1, './lib/config/Utilidades/lgbt.txt'), lvrq, twgui), id)
				})
			break
			...
			case 'geral':
				...
				nivel = await tools('others').sort(nivel[chatId], 'xp')
				try {
			...
			case 'wallpaper':
				...
				try {
					...
					let myWallp = tools('others').randVal(Object.keys(wpphe.data.data).map(z => wpphe.data.data[z].patch))
					await kill.sendFileFromUrl(from, myWallp, 'WallHaven.jpg', '<3', id)
				} catch (error) {
					await kill.reply(from, mess.noresult() + '\n\n' + mess.fail(command, error, time), id)
				}
			break
			...
			case 'email':
				...
				try {
					await kill.reply(from, mess.wait(), id)
					const mails = (await axios.get(`https://videfikri.com/api/spamemail/?email=${encodeURIComponent(arg.split('|')[0])}&subjek=${encodeURIComponent(arg.split('|')[1])}&pesan=${encodeURIComponent(arg.split('|')[2])}`)).data
					if (mails.result.status == '200') {
						await kill.reply(from, `✔️ - *📠 → *: ${mails.result.target}\n\n*📧 → * ${mails.result.subjek}\n\n*📋 → * ${mails.result.pesan}`, id)
					} else return await kill.reply(from, mess.mails(), id)
				} catch (error) {
					await kill.reply(from, mess.mails() + '\n\n' + mess.fail(command, error, time), id)
				}
			break
			...
			case 'gtav':
				if (isImage || isQuotedImage) {
					await kill.reply(from, mess.wait(), id)
					let mediaData = await decryptMedia(encryptMedia)
					const gtaUpl = await tools('cloud').upload(mediaData)
					await kill.sendFileFromUrl(from, `https://videfikri.com/api/textmaker/gtavposter/?urlgbr=${gtaUpl}`, 'Gtav.jpg', 'GTA V PS2!', id).catch(async () => {
						await kill.reply(from, mess.upfail(), id)
					})
				} else return await kill.reply(from, mess.onlyimg(), id)
			break
			...
			case 'cosplay':
				...
				await kill.sendStickerfromUrl(from, `https://rest.farzain.com/api/special/fansign/cosplay/cosplay.php?apikey=rambu&text=${encodeURIComponent(body.slice(9))}`, {
					author: config.Sticker_Author,
					pack: config.Sticker_Pack,
					keepScale: true
				})
			...
			case 'bass':
				if (isMedia && isAudio || isQuotedAudio || isPtt || isQuotedPtt) {
					let mediaData = await decryptMedia(encryptMedia)
					await tools('ffmpeg').bass(mediaData, argl[0], argl[1], argl[2], argl[3], kill, message)
			...
			case 'nightcore':
				if (isMedia && isAudio || isQuotedAudio || isPtt || isQuotedPtt) {
					let mediaData = await decryptMedia(encryptMedia)
					await tools('ffmpeg').nightcore(mediaData, argl[0], kill, message)
				} else return await kill.reply(from, mess.onlyaudio(), id)
			break
			...
			case '05':
				...
				if (tools('others').randVal(['Inocente', 'Culpado']) == 'Culpado') {
					await fs.appendFileSync('./lib/config/Utilidades/gaysreport.txt', `\n\n${genderFuck} [${await tools('others').getRandLine(1, './lib/config/Utilidades/lgbt.txt')}] - ${lvpc} Years/Anos 🔒`)
				} else await fs.appendFileSync('./lib/config/Utilidades/gaysreport.txt', `\n\n${genderFuck} [${await tools('others').getRandLine(1, './lib/config/Utilidades/lgbt.txt')}] - Innocent/inocente 🔓`)
			...
			case 'nunca':
				if (region == 'en') return await kill.reply(from, await tools('others').getRandLine(1, './lib/config/Utilidades/never.txt'), id)
				await translate(await tools('others').getRandLine(1, './lib/config/Utilidades/never.txt'), {
					to: region
				}).then(async (willdo) => {
			...
			case 'cantada':
				if (region == 'en') return await kill.reply(from, await tools('others').getRandLine(1, './lib/config/Utilidades/cantadas.txt'), id)
				await translate(await tools('others').getRandLine(1, './lib/config/Utilidades/cantadas.txt'), {
					to: region
				}).then(async (notHappy) => {
			...
			case 'bible':
				try {
					if (argl[0] == '-g') {
						let biblin = await shell.exec(`bash -c "cd lib/config/Utilidades && grep -i "${body.slice(13)}" biblia.txt | shuf -n 1"`, {
							silent: true
						}).stdout
						if (biblin == '') {
							if (region == 'en') return await kill.reply(from, await tools('others').getRandLine(1, './lib/config/Utilidades/biblia.txt'), id)
							await translate(await tools('others').getRandLine(1, './lib/config/Utilidades/biblia.txt'), {
								to: region
							}).then(async (bible) => {
					...
					} else return await translate(await tools('others').getRandLine(1, './lib/config/Utilidades/biblia.txt'), {
						to: region
					}).then(async (bible) => {
						await kill.reply(from, bible.text, id)
					})
				} catch (error) {
					await kill.reply(from, await tools('others').getRandLine(1, './lib/config/Utilidades/biblia.txt'), id)
					await tools('others').reportConsole(command, error)
				}
			break
			...
			case 'bang':
				...
				await kill.sendTextWithMentions(from, mess.lolibang(sender.id, bangme, await tools('others').getRandLine(1, './lib/config/Utilidades/armas.txt')), id)
			break
			...
			case 'news':
				...
				let graNews = await shell.exec(`bash -c "cd lib/config/Utilidades && grep -i "${body.slice(13)}" biblia.txt | shuf -n 1"`, {
					silent: true
				})
				if (graNews.stdout == '') {
			...
			case '8d':
				if (isMedia && isAudio || isQuotedAudio || isPtt || isQuotedPtt) {
					let mediaData = await decryptMedia(encryptMedia)
					await tools('ffmpeg').octasound(mediaData, argl[0], kill, message)
				} else return await kill.reply(from, mess.onlyaudio(), id)
			break
			...
			case 'forca':
				...
				if (gameconfig[chatId]['forcw'] == 0 || argl[0] == '-new') {
					gameconfig[chatId]['forcw'] = await tools('others').getRandLine(1, `./lib/config/Utilidades/${gameconfig[chatId]["forcFile"]}.txt`)
			...
			default:
				if (isCmd && prefix != "@") {
					if (Object.keys(cmds).includes(chatId) && Object.keys(cmds['global']).includes(command)) {
						return await kill.reply(from, cmds[chatId][command], id)
					} else if (Object.keys(cmds['global']).includes(command)) {
						return await kill.reply(from, cmds['global'][command], id)
					} else {
		...
		// Sistema que roda multiplos comandos de uma vez
		if (cmdtorun.length < 2 || !isOwner) break
