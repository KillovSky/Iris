...
exports.getValue = async (user, place, chatId, what) => {
	if (Object.keys(place).includes(chatId)) {
		...
		await fs.writeFileSync('./lib/config/Gerais/level.json', JSON.stringify(place, null, "\t"))
		return "0"
	}
}
...
exports.addLimit = async (user, place, what) => {
	...
	await fs.writeFileSync('./lib/config/Gerais/limit.json', JSON.stringify(place, null, "\t"))
}
