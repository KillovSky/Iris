...
exports.recognize = async (name, media, kill, message) => {
	try {
		...
		await fs.writeFile(name, media, async (err) => {
			if (err) {
				await kill.reply(message.from, 'Houve um erro de download da música.', message.id)
		...
		await tools('others').reportConsole('DETECT', error)
	}
}