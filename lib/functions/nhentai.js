	...
	try {
		...
		if (!isNaN(args[0])) {
			...
			if (await nhentai.exists(args[0])) {
				...
				await tools('others').clearFile(pasta, 1, true)
				await fs.mkdirSync(pasta)
				for (let i = 0; i < dojin.pages.length; i++) {
					const request = await https.get(dojin.pages[i], function(res) {
						res.pipe(fs.createWriteStream(`${pasta}/${i}.png`))
					})
				}
				await tools('others').sleep(3000)
				await shell.exec(`bash -c 'zip -r "${pasta}/${args[0]}.zip" ${pasta}'`, {
					silent: true
				})
				...
				await tools('others').clearFile(pasta, 120000, true)
