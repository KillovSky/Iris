...
exports.vote = async (kill, message, vote, file) => {
	if (!fs.existsSync(file)) return await kill.reply(message.from, `Ainda não existem votações abertas, crie uma usando "${prefix}NewPoll [Nome da Votação] | [Quantidade Máxima de votos]".\n\nExemplo: "${prefix}NewPoll Prefeito | 10"`, message.id)
