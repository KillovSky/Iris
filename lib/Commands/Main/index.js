...
function endLogger(kill, dataSets, commandExecuter) {
    ...
    if (
        ...
        const extraInfo = ['isGroupAdmins', 'isBlocked', 'isMedia', 'isInvite', 'isOwner', 'isVIP', 'isModerator', 'isSticker', 'isQuotedSticker', 'isAnimated', 'isQuotedAnimated', 'isAllowed', 'isBot', 'isGroupCreator', 'isCmd', 'canSticker', 'isVideo', 'isImage', 'isGroup', 'isGroupMsg', 'isQuotedMsg', 'hasViewOnce'].filter((f) => dataSets.value[f] === true);

        /* Realiza a impressão */
        console.log(
            `${Indexer('color').echo('│', Indexer('array').extract(config.colorSet.value).value).value
            + Indexer('color').echo('───────────────────────────────────────', cmdColor).value
            + Indexer('color').echo('┘\n', Indexer('array').extract(config.colorSet.value).value).value
