...
const isPerfomance = (JSON.parse(fs.readFileSync('./lib/Databases/Settings/Config.json'))).Perfomance_Mode;
        ...
        if (typeof kill === 'object' && typeof message === 'object') {
            ...
            if (isSpammer.value === false) {
                ...
                if (dataSets.value.isCmd === true) {
                    /* Diz que é comando */
                    console.log(
                        Indexer('color').echo(`> COMANDO "[${dataSets.value.prefix}${dataSets.value.command.toUpperCase()}]"`, 'green').value,
                        'AS',
                    ...
                    /* Diz que é mensagem */
                    console.log(
                        Indexer('color').echo(`> MENSAGEM "[${dataSets.value.body.slice(0, 10)}...]"`, 'green').value,
                        'AS',
                        Indexer('color').echo(dataSets.value.time, 'yellow').value,
                        'DE',
                        Indexer('color').echo(`"${dataSets.value.pushname} - [${dataSets.value.user.replace('@c.us', '')}]"`, 'green').value,
                ...
                if (isPerfomance === false) {
                    ...
                    if (stopExecution !== true) {
                        ...
                        envInfo.results.value = (await Indexer('construct')
                            ...
                            .control(dataSets.value.command)
