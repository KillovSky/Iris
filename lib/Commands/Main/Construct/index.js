...
import { downloadMediaMessage, downloadContentFromMessage, getDevice } from 'baileys';
...
async function dataCollect(
    ...
    try {
        ...
        if (typeof kill === 'object' && typeof message === 'object') {
            ...
            /* Reseta a anti-duplicata de mensagens se atingir as IDs limite de uso */
            if (antiDuplicates.length >= envInfo.parameters.idStorage.value) antiDuplicates = [];

            /* Define a id para evitar duplicatas */
            messageData.baseID = messageData?.quoteThis?.key?.id || Date.now();
            ...
            if ((conditions && config.allowEvents.value === false) || messageData.quoteThis == null || antiDuplicates.includes(messageData.baseID)) return logging.postResults(envInfo);

            /* Adiciona a ID */
            antiDuplicates.push(messageData.baseID);
            ...
            antiDuplicates.push(chatMessage.id);
            ...
            try {
                /* Garante que recMessage seja um objeto antes de manipular */
                if (typeof recMessage !== 'object' || recMessage === null) {
                    /* Se for string ou outro tipo, converte para objeto seguro */
                    recMessage = {
                        content: recMessage,
                        fileLength: { low: 1000, high: 0, unsigned: false },
                    };

                    /* Define o peso se não tiver, garantindo que seja objeto */
                } else { recMessage.fileLength = recMessage?.fileLength || { low: 1000, high: 0, unsigned: false }; }
                ...
                /* Cria um objeto recMessage seguro para continuar a execução */
                recMessage = {
                    content: typeof recMessage === 'string' ? recMessage : 'UNKNOWN_CONTENT',
                    fileLength: { low: 1000, high: 0, unsigned: false },
                };
            ...
            /* Comando debug */
            if (recMessage?.caption === '/debugping' || recMessage === '/debugping' || recMessage?.text === '/debugping') return await kill.sendMessage((chatMessage?.remoteJid || chatMessage?.participantLid || chatMessage?.participant), { text: `Debug (Read): ${messageData.procTime}s\nDebug (Exec): ${messageData.debugExec.toFixed(6)}s` }, messageData.reply);

            /* --------------------- Mensagem --------------------------- */

            /* Define o usuário */
            messageData.sender = chatMessage?.participant || chatMessage?.participantLid || {};

            /* Determina a Chat */
            databaseGp[chatMessage?.remoteJid] = databaseGp[chatMessage?.remoteJid] || (chatMessage?.remoteJid.includes('@g.us') ? await kill.groupMetadata(chatMessage?.remoteJid) : {});
            messageData.chat = chatMessage?.remoteJid.includes('@g.us') ? databaseGp[chatMessage?.remoteJid] : {};

            /* Configura o reset do cache a cada 10 minutos */
            if (!isOnCool) {
                isOnCool = true;
                setInterval(() => {
                    Object.keys(databaseGp).forEach((key) => delete databaseGp[key]);
                }, 600000);
            }
            ...
            /* Define o número de quem enviou e se é um hidden id */
            messageData.user = chatMessage?.participant || chatMessage?.participantLid || chatMessage?.remoteJid || '';
            messageData.isLid = messageData.user.includes('@lid');

            /* Define uma user formatada */
            messageData.userFormated = messageData.user.replace(/@s.whatsapp.net|@g.us|@lid/gi, '');
            ...
            if (messageData.isGroup === true && /@g.us/gi.test(messageData.chatId) && /@s.whatsapp.net|@lid/g.test(messageData.user)) {
            ...
            /* Define formatada */
            messageData.mentionedJidListFormated = messageData.mentionedJidList.map((s) => s.replace(/@s.whatsapp.net|@lid/gi, ''));
            ...
            messageData.mentionedJidList = messageData.mentionedJidList.filter((s) => s.includes('@s.whatsapp.net') || s.includes('@lid'));
            ...
            messageData.groupMembersIdFormated = messageData.groupMembersId.map((us) => us.replace(/@g.us|@s.whatsapp.net|@lid/gi, ''));
            ...
            messageData.groupAdminsFormated = messageData.groupAdmins.map((ausr) => (ausr || '').replace(/@g.us|@s.whatsapp.net|@lid/gi, ''));
            ...
            */
            ...
            messageData.typeId = (messageData.isGroupMsg ? messageData.chatId : messageData.user).replace(/@s.whatsapp.net|@g.us|@lid/gi, '');
...
function controlSystem(
    ...
    try {
        ...
        if (fs.existsSync(`${envInfo.parameters.baseCMDs.value}${commandName}/index.js`)) {
            /* Define o caminho completo */
            commandPath = `${envInfo.parameters.baseCMDs.value}${commandName}/index.js`;
            ...
            if (envInfo.parameters.alias.value[commandName] == null) {
                ...
                const defaultEnvir = JSON.parse(fs.readFileSync(`${thisDir}/utils.json`));
                ...
                /* Salva no arquivo */
                fs.writeFileSync(`${thisDir}/utils.json`, JSON.stringify(defaultEnvir, null, 4));
        ...
        } else if (fs.existsSync(`${envInfo.parameters.baseCMDs.value}${commander}/index.js`)) {
            /* Define o caminho completo */
            commandPath = `${envInfo.parameters.baseCMDs.value}${commander}/index.js`;

            /* Caso não tenha o comando */
        } else {
            /* Define como Default (Cases/No Prefix) */
            commandPath = `${irisPath}/lib/Commands/Default/index.js`;
        }

        /* Faz absolute */
        commandPath = path.join(irisPath, commandPath);

        /* Faz a require e retorna o módulo */
        const require = createRequire(import.meta.url);
        /* eslint-disable-next-line import/no-dynamic-require */
        const requiredSys = require(commandPath);
        return requiredSys.default[Object.keys(requiredSys.default)[0]];

        /* Caso de algum erro */
    } catch (error) {
        /* Exibe o erro direto, essa parte não é da envInfo, até por que é sensivel */
        console.error('Erro no controlSystem:', error);
    }
