...
async function dataCollect(
    ...
    try {
        ...
        if (typeof kill === 'object' && typeof message === 'object') {
            ...
            const hasViewOnce = messageData.quoteThis.message?.viewOnceMessageV2 || messageData.quoteThis.message?.viewOnceMessage || recMessage?.viewOnce;
            /* eslint-disable-next-line no-nested-ternary */
            const viewOnce = (hasViewOnce && viewOnceObj) ? viewOnceObj[Object.keys(viewOnceObj)[0]] : (hasViewOnce ? recMessage : false);
            messageData.viewOnce = viewOnce;
            messageData.hasViewOnce = hasViewOnce;
            ...
            messageData.decryptedMedia = messageData.isCmd && messageData.decryptFormats.includes(messageData.typeFormatted) && ((recMessage?.fileLength?.low || recMessage?.fileLength?.high || 1000) / 1024) <= config.maxDecryptSize.value ? await downloadMediaMessage(messageData.quoteThis, 'buffer') : '';
            ...
            */
            ...
            if ((messageData.isCmd && messageData.quotedMsgObj?.mimetype && !messageData.isMedia) || (messageData.isCmd && hasViewOnce)) {
                ...
                if (messageData.decryptFormats.includes(Object.keys(quotMessage)[0].replace('Message', ''))) {
                    ...
                    if (((recMessage?.fileLength?.low || recMessage?.fileLength?.high || 1000) / 1024) <= config.maxDecryptSize.value) {
                        ...
                        /* eslint-disable-next-line no-nested-ternary */
                        messageData.encryptMedia = (hasViewOnce && viewOnceObj) ? viewOnce[Object.keys(viewOnce)[0]] : (hasViewOnce ? messageData.recMessage : messageData.quotedMsgObj);
