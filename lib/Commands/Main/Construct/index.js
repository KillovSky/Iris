...
async function dataCollect(
    ...
    try {
        ...
        if (typeof kill === 'object' && typeof message === 'object') {
            ...
            /* Define se deve ignorar as mensagens da Íris */
            if (((chatMessage?.fromMe && config.botCommands.value === false)) || (messageData.quoteThis.broadcast === true && config.listenBroadcasts.value === false) || (Object.keys(messageData.quoteThis?.message).includes('reactionMessage') && config.listenReactions.value === false)) {
                /* Define soft error */
            ...
            messageData.decryptedMedia = messageData.decryptFormats.includes(messageData.typeFormatted) && !messageData.quoteThis.message.viewOnceMessageV2 ? await downloadMediaMessage(messageData.quoteThis, 'buffer') : '';
            ...
            messageData.user = chatMessage?.participant || messageData.sender || chatMessage?.remoteJid || '';
            ...
            */
            ...
            if ((messageData.quotedMsgObj?.mimetype && !messageData.isMedia) || isViewOnce) {
...
function controlSystem(
    ...
    try {
        ...
        if (fs.existsSync(`${envInfo.parameters.baseCMDs.value}${commandName}`)) {
            ...
            const folderAddiction = fs.readdirSync(envInfo.parameters.baseCMDs.value).filter((f) => f.toLowerCase() === commandName)[0] || commandName;

            /* Só edita se não existir */
            if (envInfo.parameters.alias.value[folderAddiction] == null) {
                /* Define o aliás automatico */
                const defaultEnvir = JSON.parse(fs.readFileSync(`${__dirname}/utils.json`));
                defaultEnvir.parameters.alias.value[folderAddiction] = defaultEnvir.parameters.alias.value[folderAddiction] || [commandName];
