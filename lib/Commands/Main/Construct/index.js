...
async function dataCollect(
    ...
    try {
        ...
        if (typeof kill === 'object' && typeof message === 'object') {
            ...
            if (!typeof messageData.quoteThis?.message === 'object' || messageData.quoteThis?.message == null || messageData.quoteThis == null || message.type === 'append') return postResults(envInfo.results);
            ...
            messageData.name = (messageData.chat?.subject || '').replace(/[`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/]/gi, '');
            ...
            // eslint-disable-next-line no-useless-escape
            messageData.pushname = (message?.messages[0]?.pushName || '"Censored by Government"').replace(/[`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/]/gi, '');
