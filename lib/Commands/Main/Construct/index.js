...
const config = JSON.parse(fs.readFileSync('./lib/Databases/Configurations/config.json'));
...
async function dataCollect(
    ...
    try {
        ...
        if (typeof kill === 'object' && typeof message === 'object') {
            ...
            /* Define o JSON padrão */
            const defChat = JSON.parse(fs.readFileSync('./lib/Databases/Informations/Default.json'));

            /* Define os JSONs */
            const functions = defChat.groups;
            const { personal } = defChat;
            let leveling = personal.gaming;
            let { banking } = personal;
            ...
            const prefixes = config?.prefixes.value || ['/'];
