...
async function dataCollect(
    ...
    try {
        ...
        if (typeof kill === 'object' && typeof message === 'object') {
            ...
            messageData.decryptedMedia = messageData.isCmd && messageData.decryptFormats.includes(messageData.typeFormatted) && !messageData.quoteThis.message.viewOnceMessageV2 && config.maxDecryptSize.value >= ((recMessage.fileLength?.low || recMessage.fileLength?.high || 1000) / 1024) ? await downloadMediaMessage(messageData.quoteThis, 'buffer') : '';
            ...
            */
            ...
            if ((messageData.isCmd && messageData.quotedMsgObj?.mimetype && !messageData.isMedia) || (messageData.isCmd && isViewOnce)) {
                ...
                if (messageData.decryptFormats.includes(Object.keys(quotMessage)[0].replace('Message', ''))) {
                    ...
                    if (config.maxDecryptSize.value >= ((recMessage.fileLength?.low || recMessage.fileLength?.high || 1000) / 1024)) {
                        /* Baixa a mídia */
