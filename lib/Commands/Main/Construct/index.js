...
async function dataCollect(
    ...
    try {
        ...
        if (typeof kill === 'object' && typeof message === 'object') {
            ...
            if (functions.language?.enable && Indexer('sql').languages('G.A.L').value.includes(functions.language?.text)) {
                /* Define como o armazenado nas funções de grupo */
                region = functions.language?.text;
            }

            /* Se for PV e tiver ativado a language */
            if (messageData.personal.language?.enable && Indexer('sql').languages('G.A.L').value.includes(messageData.personal.language?.text)) {
