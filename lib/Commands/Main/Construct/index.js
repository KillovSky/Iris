...
async function dataCollect(
    ...
    try {
        ...
        if (typeof kill === 'object' && typeof message === 'object') {
            ...
            let keysIndex = Object.keys(messageData.quoteThis.message).filter((f) => f !== 'senderKeyDistributionMessage' && f !== 'messageContextInfo' && !f.includes('Message'));
            keysIndex[0] = keysIndex[0] || 'conversation';
            let recMessage = messageData.quoteThis.message[keysIndex[0]];

            /* Reorganiza a recMessage se precisar */
            keysIndex = Object.keys(recMessage).filter((f) => f !== 'senderKeyDistributionMessage' && f !== 'messageContextInfo' && !f.includes('Message'));
            keysIndex[0] = keysIndex[0] || 'conversation';
