...
import gtts from '@killovsky/gtts';
import cleverbot from 'cleverbot-free';
import simsimi from 'chats-simsimi';
import { fileURLToPath } from 'url';
import gpt4all from '../../Scripts/IAs/Chatbots/gpt4all.js';
import transformers from '../../Scripts/IAs/Chatbots/transformers.js';
        ...
        if (typeof kill === 'object' && typeof env === 'object') {
            ...
            if (arks.includes('--help-dev') && isOwner === true) {
                ...
                if (arks.includes('-tts') || command === 'tts') {
                    /* Cria um buffer */
                    const speakdata = await gtts.create(region, sendRes.text, true, `${path.normalize(`${thisDir}/Cache/`)}`);

                    /* Define o arquivo de saida */
                    const exitFile = `${path.normalize(`${thisDir}/Cache/${Indexer('strings').generate(10)}.mp3`)}`;
