    ...
    try {
        ...
        if (typeof kill === 'object' && typeof env === 'object') {
            ...
            if (arks.includes('--help-dev') && isOwner === true) {
                ...
                const randomRes = Indexer('bash').liner(1, `${`${path.normalize(irisPath)}/lib/Databases/Utilities/chat.txt`}`, customSearch).value;
                ...
                try {
                    ...
                    if (arks.includes('-cleverbot') && command !== 'tts') {
                        ...
                        if (gptText.success) {
                            /* Insere na object */
                            sendRes = { text: `${gptText.value.trim()}\n\nThis GPT can be slow and dumber than the others, as it is a simplified version for bad computers, do not abuse it and wait patiently for a response, use english only.` };
                        }
                    }
