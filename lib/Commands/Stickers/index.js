...
async function resizeVideo(mediaData, stickerSettings) {
    ...
    return new Promise((resolve) => {
        ...
        const cacheSticker = path.normalize(`${thisDir}/Cache/${Indexer('strings').generate(10).value}.webp`);
        const stickerLocation = path.normalize(`${thisDir}/Cache/${Indexer('strings').generate(10).value}.webp`);
        ...
        (ffmpeg().input(cacheSticker).noAudio()
            ...
            .on('error', (err) => {
                ...
                resolve(path.normalize(fs.readFileSync(`${thisDir}/Cache/420.jpg`)));
            })
