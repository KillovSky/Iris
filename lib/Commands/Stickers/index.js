...
async function stickerMaker(
    ...
    try {
        ...
        if (typeof kill === 'object' && typeof env === 'object') {
            ...
            stickerConfig.circle = !!arks.includes('-circle');
            stickerConfig.keepScale = !arks.includes('-cut');
            ...
            if (canSticker === true) {
                ...
                if (isVideo === true) {
                    /* Se video, faz o envio usando a conversão de MP4 */
                    envInfo.results.value = await kill.sendMp4AsSticker(chatId, mediaData, null, stickMp4Config);

                    /* Se for imagem ou qualquer coisa além de video */
                } else {
                    /* Faz o envio usando a conversão de imagem */
                    envInfo.results.value = await kill.sendImageAsSticker(chatId, mediaData, stickerConfig);
                }

                /* Caso tenha uma URL */
            } else if (Indexer('regexp').urls(args[0]).value) {
                /* Usa a função de sticker por URL */
                envInfo.results.value = await kill.sendStickerfromUrl(chatId, args[0], {
                    method: 'get',
                }, stickerConfig);
