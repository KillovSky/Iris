...
async function resizeVideo(mediaData) {
    /* Ffmpeg não possui suporte nativo a Promises, portanto, criamos uma para encapsular a operação assíncrona */
    return new Promise((resolve) => {
        /* Define o local do Sticker */
        const cacheSticker = path.normalize(`${__dirname}/Cache/${Indexer('strings').generate(10)}.webp`);
        const stickerLocation = path.normalize(`${__dirname}/Cache/${Indexer('strings').generate(10)}.webp`);

        /* Baixa o sticker */
        fs.writeFileSync(cacheSticker, mediaData);

        /* Configura o ffmpeg para redimensionar o vídeo para 512x512 pixels */
        (ffmpeg().input(cacheSticker).size('512x512')
            /* Opções para tirar o máximo de peso possivel do video */
            .noAudio()
            .outputOptions('-fs 1M')

            /* Quando o redimensionamento é concluído com sucesso */
            .on('end', () => {
                /* Limpa o cache */
                Indexer('clear').destroy(cacheSticker);

                /* resolve a Promise com o local do sticker */
                resolve(stickerLocation);
            })
            ...
            .on('error', () => {
                ...
                resolve(path.normalize(`${__dirname}/Cache/420.jpg`));
            })
...
async function stickerMaker(
    ...
    try {
        ...
        if (typeof kill === 'object' && typeof env === 'object') {
            ...
            if ((canSticker === true && env.value.decryptedMedia) || Indexer('regexp').urls(args[0]).value.isURL) {
                ...
                let mediaData = canSticker === true && env.value.decryptedMedia ? env.value.decryptedMedia : args[0];

                /* Se for um video */
                if (isVideo) {
                    /* Redimensiona para ficar com peso menor */
                    mediaData = await resizeVideo(mediaData);
                }
                ...
                if (isVideo) {
                    /* Limpa ele */
                    Indexer('clear').destroy(mediaData);
                }
