 ...
 * @param {string} isLid - Define se deve enviar a mensagem para um lid.
 ...
 */
async function checkGaming(chessg, kill, chatId, gameCreator, reply, nowPlayer, rivalPlayer, isLid) {
    ...
    if (inMemoryGame[chatId][gameCreator] !== null) {
        ...
        let dialogue = '';
        const userPlayer = isLid ? `${nowPlayer}@lid` : `${nowPlayer}@s.whatsapp.net`;
        ...
        if (['nomoreactions', 'stalemate', 'draw', 'gameover', 'threefoldrepetition'].includes(result)) {
            ...
            /* Define o dialogo */
            dialogue = Indexer('sql').languages(region, 'Games', 'Victory', true, true, { gameend: result, winner: userPlayer.replace(/@s.whatsapp.net|@lid/gi, ''), looser: rivalPlayer.replace(/@s.whatsapp.net|@lid/gi, '') }).value;
        }
                ...
                switch (argl[0]) {
                    ...
                    case '-board':
                        ...
                        if (!Object.keys(inMemoryGame[chatId]).includes(gameCreator)) {
                            ...
                            await kill.sendMessage(chatId, { image: { url: `${thisDir}/chess.png` }, caption: `🔢 Moves: ${createChess.moves().join(', ')}\n\n📊 FEN:\n${inMemoryGame[chatId][gameCreator].fen}` }, reply);
                        }
                    break;
                    ...
                    case '-placar':
                        ...
                        if (Object.keys(inMemoryGame.score).length === 0) {
                            ...
                            const scoreboard = Object.keys(inMemoryGame.score).sort((a, b) => inMemoryGame.score[b].win - inMemoryGame.score[a].win).map((player) => {
                                ...
                                playerNumber = playerNumber.value.name.text === 'default' ? player.replace(/@s.whatsapp.net|@lid/gi, '') : playerNumber.value.name.text;
                    ...
                    case '-create':
                        ...
                        if (Object.keys(inMemoryGame[chatId]).includes(gameCreator)) {
                            ...
                            inMemoryGame[chatId][gameCreator].playerOneFormatted = gameCreator.replace(/@s.whatsapp.net|@lid/gi, '');
                            inMemoryGame[chatId][gameCreator].playerTwoFormatted = mentionsPlayer[0].replace(/@s.whatsapp.net|@lid/gi, '');
                    ...
                    case '-play':
                        ...
                        if (!Object.keys(inMemoryGame[chatId]).includes(gameCreator)) {
                            ...
                            const playerFormatted = actualPlayer.replace(/@s.whatsapp.net|@lid/gi, '');
                            ...
                            if (inMemoryGame[chatId][gameCreator].currentStep !== user) {
                                ...
                                await kill.sendMessage(chatId, { image: { url: `${thisDir}/chess.png` }, caption: Indexer('sql').languages(region, 'Games', 'Invalid', true, true, { movements: chessGame.moves().join(', ') }).value }, reply);
                                ...
                                /* Verifica se venceu */
                                const isVictory = await checkGaming(chessGame, kill, chatId, gameCreator, reply, playerFormatted, rivalPlayer, isLid);

                                /* Se for draw */
                                if (isVictory === false) {
                                    /* Formata o user para mencionar */
                                    const mentionRival = inMemoryGame[chatId][gameCreator].currentStep.replace(/@s.whatsapp.net|@lid/gi, '');
