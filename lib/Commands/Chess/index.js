...
async function checkGaming(chessg, kill, chatId, gameCreator, quoteThis, nowPlayer, rivalPlayer) {
    ...
    if (inMemoryGame[chatId][gameCreator] !== null) {
        ...
        if (['nomoreactions', 'stalemate', 'draw', 'gameover', 'threefoldrepetition'].includes(result)) {
            ...
            /* Define o dialogo */
            dialogue = Indexer('sql').languages(region, 'Games', 'Victory', true, true, { gameend: result, winner: userPlayer.replace(/@s.whatsapp.net/gi, '').value, looser: rivalPlayer.replace(/@s.whatsapp.net/gi, '') });
        }
                ...
                switch (argl[0]) {
                    ...
                    case '-board':
                        ...
                        if (!Object.keys(inMemoryGame[chatId]).includes(gameCreator)) {
                            ...
                            await kill.sendMessage(chatId, { text: Indexer('sql').languages(region, 'Games', 'Uncreated', true, true, env.value).value }, { quoted: quoteThis });
                    ...
                    case '-placar':
                        ...
                        if (Object.keys(inMemoryGame.score).length === 0) {
                            /* Diz que já pode criar */
                            await kill.sendMessage(chatId, { text: Indexer('sql').languages(region, 'Games', 'NoPlacar', true, true, env.value).value }, { quoted: quoteThis });
                    ...
                    case '-create':
                        ...
                        if (Object.keys(inMemoryGame[chatId]).includes(gameCreator)) {
                            ...
                            await kill.sendMessage(chatId, { text: Indexer('sql').languages(region, 'Games', 'Created', true, true, env.value).value, mentions: [user, mentionsPlayer[0]] }, { quoted: quoteThis });
                    ...
                    case '-cancel':
                        ...
                        if (!Object.keys(inMemoryGame[chatId]).includes(gameCreator)) {
                            ...
                            /* Avisa que apagou */
                            await kill.sendMessage(chatId, { text: Indexer('sql').languages(region, 'Games', 'Delete', true, true, env.value).value }, { quoted: quoteThis });
                        }
                    break;
                    ...
                    case '-play':
                        ...
                        if (!Object.keys(inMemoryGame[chatId]).includes(gameCreator)) {
                            ...
                            if (inMemoryGame[chatId][gameCreator].currentStep !== user) {
                                ...
                                await kill.sendMessage(chatId, { text: Indexer('sql').languages(region, 'Games', 'NotTurn', true, true, { playerFormatted }).value, mentions: [inMemoryGame[chatId][gameCreator].playerOne, inMemoryGame[chatId][gameCreator].playerTwo] }, { quoted: quoteThis });

                                /* Se for a vez, mas a jogada não for válida */
                            } else if (playerStep === false) {
                                /* Envia que a jogada é invalida e fornece ajuda */
                                await kill.sendMessage(chatId, { image: { url: `${__dirname}/chess.png` }, caption: Indexer('sql').languages(region, 'Games', 'Invalid', true, true, { movements: chessGame.moves().value.join(', ') }) }, { quoted: quoteThis });
