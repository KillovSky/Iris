                    ...
                    case '-play':
                        ...
                        if (!Object.keys(inMemoryGame[chatId]).includes(gameCreator)) {
                            ...
                            if (inMemoryGame[chatId][gameCreator].currentStep !== user) {
                                ...
                                await kill.sendMessage(chatId, { image: { url: `${__dirname}/chess.png` }, caption: Indexer('sql').languages(region, 'Games', 'Invalid', true, true, { movements: chessGame.moves().join(', ') }).value }, { quoted: quoteThis });
