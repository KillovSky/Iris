...
function generateRandomString(length) {
    ...
    let result = '';
    for (let i = 0; i < length; i + 1) { // Itera pelo comprimento desejado para criar a string aleatória.
...
function extractURL(text) {
    const urlRegex = /(https?:\/\/[^\s]+)/; // Expressão regular para encontrar URLs em um texto.
    const matches = text.match(urlRegex); // Tenta encontrar uma correspondência da URL usando a expressão regular.
    ...
    return null; // Retorna null se nenhuma URL for encontrada
}
