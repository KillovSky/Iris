        ...
        if (typeof kill === 'object' && typeof env === 'object') {
            ...
            switch (commandFound) {
                ...
                */
                ...
                case caseChecker('NO PREFIX 123 +@', false, isCmd):
                    envInfo.results.value = await kill.sendMessage(chatId, { text: Indexer('sql').languages(region, 'Cases', 'Standard', true, true, env.value).value }, { quoted: quoteThis });
                break;
                ...
                */
                default:
                    if (isCmd === true) {
                        envInfo.results.value = await kill.sendMessage(chatId, { text: Indexer('sql').languages(region, 'Cases', 'Test', true, true, env.value).value }, { quoted: quoteThis });
                    }
                break;
            }
        }
