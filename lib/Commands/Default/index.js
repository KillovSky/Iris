            ...
            switch (commandFound) {
                ...
                */
                case 'eval':
                    ...
                    if (isOwner === true) return eval(env.value.arg);
                break;
                ...
                */
                case 'menu':
                    envInfo.results.value = await kill.sendMessage(chatId, {
                        text: Indexer('bash').bash(`bash "${irisPath}/lib/Scripts/Menu.sh" "${argl[0]}" ${JSON.stringify(language(region, 'Helper', 'Menu', true, true, env.value))}`).value,
                    }, { quoted: quoteThis });
                break;
