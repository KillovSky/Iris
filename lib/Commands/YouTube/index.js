    ...
    try {
        ...
        if (Indexer('regexp').urls(linkURL).value.isURL) {
            ...
            if (searchType === 'audio') {
                ...
                downOptions.o = path.normalize(`${thisDir}/Cache/${Indexer('strings').generate(10).value}.mp3`);
            }
            ...
            if (searchType === 'audio' && youTubeMedia != null && youTubeMedia !== 'dontDownload') {
                ...
                const newAudioFile = path.normalize(`${thisDir}/Cache/${Indexer('strings').generate(10).value}.ogg`);
