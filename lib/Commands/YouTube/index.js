...
function fixForIOS(inputPath, outputPath) {
    ...
    try {
        ...
        if (Indexer('regexp').urls(linkURL).value.isURL) {
            ...
            if (searchType === 'audio') {
                ...
                downOptions.o = path.normalize(`${__dirname}/Cache/${Indexer('strings').generate(10)}.mp3`);
            }
            ...
            if (searchType === 'audio' && youTubeMedia != null && youTubeMedia !== 'dontDownload') {
                ...
                const newAudioFile = path.normalize(`${__dirname}/Cache/${Indexer('strings').generate(10)}.ogg`);
...
async function youTubeDownloader(
    ...
    try {
        ...
        if (typeof kill === 'object' && typeof env === 'object') {
            ...
            /* Ajusta a body */
            const body = env.value.body.replace(/-audio|-video|-link/g, '');
