    ...
    try {
        ...
        if (Indexer('regexp').urls(linkURL).value.isURL) {
            ...
            if (youTubeMedia == null || youTubeMedia === 'dontDownload' || envFinish.value === 'dontDownload' || envFinish.value == null) {
                /* Avisa a pessoa */
                await kill.sendMessage(chatId, { text: Indexer('sql').languages(region, 'Play', 'Failed', true, true, envInfo).value }, { quoted: quoteThis });
        ...
        if (typeof kill === 'object' && typeof env === 'object') {
            ...
            if (arks.includes('--help-dev') && isOwner === true) {
                ...
                if (canAdvise === true && command !== 'ytsearch' && command !== 'videosearch') {
                    ...
                    if (config.waitMessage.value) await kill.sendMessage(chatId, { text: Indexer('sql').languages(region, 'Extras', 'Wait', true, true, envInfo).value }, { quoted: quoteThis });
                }
                ...
                if ((youTubeData.value.length !== 0 && youTubeData.success === true && Array.isArray(youTubeData.value)) || haveURL.isURL === true) {
                    ...
                    if (downFormat !== 'video' && !arks.includes('-link') && haveURL.isURL === false) {
                        ...
                        await kill.sendMessage(chatId, { image: { url: youTubeData.value.image }, caption: Indexer('sql').languages(region, 'Play', 'Details', true, true, youTubeData.value).value }, { quoted: quoteThis });
                    }
                    ...
                    if (
                        ...
                        if (arks.includes('-link')) {
                            ...
                            await kill.sendMessage(chatId, { video: { url: baixarYouTube.value }, mimetype: 'video/mp4', caption: Indexer('sql').languages(region, 'Play', 'Details', true, true, youTubeData.value).value }, { quoted: quoteThis });
                    ...
                    envInfo.results.value = await kill.sendMessage(chatId, { text: Indexer('sql').languages(region, 'Play', 'Empty', true, true, envInfo).value }, { quoted: quoteThis });
                }
            }
        }
