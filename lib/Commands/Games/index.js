    ...
    try {
        ...
        if (typeof kill === 'object' && typeof env === 'object') {
            ...
            if (prisionData[user] !== 0) {
                ...
                /* Manda marcar alguém */
                envInfo.results.value = await kill.sendMessage(chatId, { text: Indexer('sql').languages(region, 'Cases', 'None', true, true, envInfo).value }, { quoted: quoteThis });
                ...
                switch (command) {
                    ...
                    case 'roubar':
                        ...
                        if (stealMatterie.length === 0) {
                            ...
                            envInfo.results.value = await kill.sendMessage(chatId, { text: Indexer('sql').languages(region, 'Games', 'noMoney', true, true, env.value).value }, { quoted: quoteThis });
                        ...
                        if (payCops <= 0) {
                            ...
                            if (paymentCops.length === 0) {
                                /* Fala que essa pessoa não rola */
                                envInfo.results.value = await kill.sendMessage(chatId, { text: Indexer('sql').languages(region, 'Games', 'noSteal', true, true, { prisonTime: envInfo.parameters.prisonTime.value }).value }, { quoted: quoteThis });
                        ...
                        if (lvpc <= stealChance) {
                            /* Avisa que deu certo */
                            envInfo.results.value = await kill.sendMessage(chatId, { text: Indexer('sql').languages(region, 'Games', 'winSteal', true, true, { sorryPeople: sorryPeople.replace(/@s.whatsapp.net/gi, '').value, stealQuantity, stealMatterie }), mentions: [user, sorryPeople] }, { quoted: quoteThis });
                            ...
                            envInfo.results.value = await kill.sendMessage(chatId, { text: Indexer('sql').languages(region, 'Games', 'lostSteal', true, true, { payCops, paymentCops, victim: `@${sorryPeople.replace(/@s.whatsapp.net/gi, '').value}` }), mentions: [user, sorryPeople] }, { quoted: quoteThis });
