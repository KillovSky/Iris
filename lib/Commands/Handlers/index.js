...
async function functionsEditor(
    ...
    try {
        ...
        if (typeof kill === 'object' && typeof env === 'object') {
            ...
            const type = Object.keys(functions).filter((s) => s.toLowerCase() === typeBase && s !== 'error')[0] || 'none';
            ...
            if (conditions === true) {
                ...
                if (arks.includes('--help') || type === 'handlers') {
                    ...
                    if (functions[type]?.enable === enaDis) {
                        ...
                        Indexer('sql').update('groups', user, chatId, type, { enable: true });
                    ...
                    /* Adquire os valores iniciais */
                    const settedValues = Indexer('sql').get('groups', user, chatId);
                    ...
                    if (settedValues[type] !== null) {
                        /* If's para assimilar os valores corretamente de acordo com a necessidade */
                        if (settedValues[type].values !== null) {
                            /* Define se é troca, remover ou adicionar */
                            let newArraye = [];

                            /* 1 - Adicionar */
                            if (argl.includes('add')) newArraye = [...settedValues[type].values, ...theCode];

                            /* 2 - Remover */
                            else if (argl.includes('remove')) newArraye = settedValues[type].values.filter((d) => !theCode.includes(d));
                    ...
                    /* Adquire os valores iniciais */
                    const sqlvalue = Indexer('sql').get('groups', user, chatId);
                    ...
                    if (['vips', 'mods', 'blacklist', 'prefix', 'whitelist', 'antifake'].includes(type) && argl[0] !== '-text') {
                        ...
                        if (argl[0] === '-add') {
                            ...
                            Indexer('sql').update('groups', user, chatId, type, { values: [...sqlvalue[type][argl[1]], ...theCode] });

                            /* Determina se deve remover */
                        } else if (argl[0] === '-rem') {
                            /* Remove da lista */
                            Indexer('sql').update('groups', user, chatId, type, { values: sqlvalue[type].values.filter((d) => d !== argl[1]) });
                        ...
                        /* Define o novo body */
                        const bodhy = body.replace(/-text|-add|-rem|-clear/g, '').replace(command, '').replace(/ {2}/g, '').replace(/^ /g, '');
