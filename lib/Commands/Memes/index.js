...
/* Cria a função de comando, se o comando for memes converte em gay */
async function memeCreator(
    ...
    try {
        ...
        if (typeof kill === 'object' && typeof env === 'object') {
            ...
            if (arks.includes('--help-dev') && isOwner === true) {
                ...
                let profilePictures = ['https://thispersondoesnotexist.com'];

                /* Se não for um meme de texto */
                if (!['cmm', 'ohno', 'clyde'].includes(command.toLowerCase())) {
                    /* Baixa as imagens corretamente */
                    profilePictures = await Indexer('profile').perfil(kill, env);
                }

                /* Define a imagem */
                const theCanvedImage = await Indexer('memes').memes(command, profilePictures.value, env);

                /* Define se vai mandar como sticker */
                if (argl.includes('-sticker')) {
...
function resetAmbient(
    ...
    try {
        ...
        /* Insere a memeCreator na envInfo */
        envInfo.functions.exec.value = memeCreator;
