...
async function genSession(launchOpt, i, launchInstancer) {
    ...
    /* Com base no nome da sessão configurado | Definir a versão separado, senão não funciona */
    const kill = await makeWASocket({ version: startOptions.version, ...startOptions });
...
async function generateSessions() {
    ...
    try {
        ...
        const startOptions = await Indexer('options').create();

        /* Inicia as sessões */
        for (let i = 0; i < launchInstance.sessionsLength; i += 1) {
            /* Executa a geração da sessão */
            const client = await genSession(startOptions.value, i, launchInstance);
