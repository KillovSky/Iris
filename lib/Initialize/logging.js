...
const loggingFile = `${global.irisPath}/logs/${(new Date().toISOString()).replace(/:/gi, '-')}.log`;

/* Define se o log inicial foi feito e como usar o Command */
const commandExists = commandExistsModule.sync;
let firstStart = true;
 ...
 *
 * @param {string} scriptDir O caminho absoluto onde o script está (geralmente `thisDir`).
...
function envInfoGenerator(envData = {}) {
    ...
    return {
        ...
        helps: customConfig.helps || [
            "Você pode mudar os parâmetros da exports enviando o valor da Object que deseja editar ao resetar, por exemplo → NomeDoComando.reset({ name: 'Body'}) ← Isso mudaria o module de NomeDoComando para Body, o uso então passaria ser: → Body.funcão ← Isso também permite que você edite a função usando o same método.",
        ...
        modules: customConfig.modules || {
            ...
            '../../index.js': 'Para rodar funções de outros arquivos.',
        },
 ...
 * @throws {Error} Se houver falha ao leer/processar o arquivo de configuração
 *
 */
    ...
    try {
        ...
        if (typeof jsonData === 'object' && jsonData != null) {
            ...
            try {
                ...
                global.logging.echoError(error, envInfo, import.meta.url);
            }
        }
        ...
        global.logging.echoError(error, envInfo, dirStr);
    }

    /* Constrói o retorno para módule */
    const returnData = {};
