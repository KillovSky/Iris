...
function dialoguePicker(
    ...
    try {
        ...
        let userLang = (typeof ulan === 'string' ? ulan.toLowerCase() : 'pt');

        /* Redefine a object */
        const objRepl = objMan;
        ...
        if (dtform instanceof Error && dtype === 'S.E.R') {
            /* Define o dialogo raiz */
        ...
        if (onlyOne === true) {
            ...
            return allDial[0].replace(/\{([a-zA-Z0-9.]+)\}/gi, (match, key) => Indexer('others').repl(objRepl, match, key));
        }
