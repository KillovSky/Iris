...
/* Imports - Importações essenciais para o funcionamento do módulo */
import fs from 'fs';
...
/* Faz import do logging se não tiver */
if (!(typeof global.logging === 'function')) {
    import('./Initialize/logging.js').then((module) => {
        global.logging = module.default;
    }).catch((err) => {
        console.error('Erro ao importar logging:', err);
    });
}
...
const commandCache = new LRUCache({
    ...
    sizeCalculation: (value) => {
        /* Verifica se o valor é undefined/null */
        if (value === undefined || value === null) return 1;
        try { return JSON.stringify(value)?.length || 1; } catch { return 1; }
    },
    ttl: 1000 * 60 * 15,
});
...
/* Configurações globales - Carregamento seguro com fallback */
...
/* Mapa para rastrear caminhos de arquivos para módulos */
const filePathToModuleMap = new Map();
...
function resolveCommandPath(commandFolder) {
    ...
    const commandPath = path.resolve(import.meta.dirname, commandPlaces[folder].place);
    return safePathJoin(import.meta.dirname, path.normalize(commandPath));
}

/**
 * Obtém o nome do módulo a partir do caminho do arquivo
 * @param {string} filePath - Caminho do arquivo
 * @returns {string} Nome do módulo correspondente
 */
function getModuleNameFromFilePath(filePath) {
    /* Converte em array e tenta localizar */
    const foundModule = (Array.from(filePathToModuleMap.entries())
        .find(([, modulePath]) => modulePath === filePath)
    );

    /* Se encontrar retorna o valor */
    if (foundModule) return foundModule[0];
...
/**
 * Carrega o módulo do comando com cache de forma síncrona
 * @param {string} commandPath - Caminho do módulo
 * @param {string} moduleName - Nome do módulo para mapeamento
 * @returns {Object} Módulo carregado
 * @throws {Error} Se o carregamento falhar
 */
function loadCommandModuleSync(commandPath, moduleName) {
    /* Circuit Breaker: Verifica se o módulo está em estado de falha */
    if (loadCommandModuleSync.failedModules.has(commandPath)) {
    ...
    try {
        ...
        /* Define a contagem de performance atual */
        const start = performance.now();

        /* Usa require para carregamento síncrono */
        const require = createRequire(import.meta.url);
        const requiredSys = require(commandPath);
        const module = requiredSys.default[Object.keys(requiredSys.default)[0]];

        /* Mapeia o caminho do arquivo para o nome do módulo */
        filePathToModuleMap.set(moduleName, commandPath);

        /* Insere em cache e registra performance */
        ...
        loadCommandModuleSync.failedModules.set(commandPath, Date.now());
        throw err;
    }
}

/* Cria um cache para os módulos que falharam */
loadCommandModuleSync.failedModules = new Map();
 ...
 * @throws {Error} Se o carregamento falhar
 */
async function loadCommandModuleAsync(commandPath, moduleName) {
    /* Circuit Breaker: Verifica se o módulo está em estado de falha */
    if (loadCommandModuleSync.failedModules.has(commandPath)) {
        throw new Error(`Módulo ${commandPath} está em estado de falha`);
    }
    ...
    try {
        /* Verificação de cache com LRU */
        if (commandCache.has(commandPath)) {
            /* Se o módulo estiver no cache, retorna do cache */
        ...
        /* Se ocorrer erro ao carregar o módulo */
    } catch (err) {
...
function handleSystemError(error, context = '') {
    ...
    if (global.logging && global.logging.echoError) {
        ...
        global.logging.echoError(error, false, import.meta.dirname);
    }
}

/**
 * Controlador principal do sistema (SÍNCRONO)
...
function controlSystem(systemName = 'Default') {
    ...
    try {
        ...
        /* Carrega o módulo usando o carregador síncrono */
        const module = loadCommandModuleSync(commandPath, systemName);
...
/**
 * Versão assíncrona do controlador
 * @param {string} systemName - Nome do sistema/comando
 * @returns {Promise<Object|boolean>} Promise com o módulo ou false
 */
async function controlSystemAsync(systemName = 'Default') {
    /* Define o início da contagem de performance */
    const startTime = performance.now();
    ...
    try {
        ...
        /* Carrega o módulo de forma assíncrona */
        const module = await loadCommandModuleAsync(commandPath, systemName);
...
/**
 * Limpa o cache de um módulo específico ou de todos os módulos
 * @param {string} [moduleName] - Nome do módulo a ser limpo (opcional)
 */
function clearCache(moduleName = null) {
    /* Se tiver um nome específico para limpar */
    if (moduleName) {
        /* Obtém o caminho do arquivo do módulo */
        const filePath = filePathToModuleMap.get(moduleName);

        /* Se ele existir */
        if (filePath) {
            /* Remove do cache LRU e do cache de falhas */
            commandCache.delete(filePath);
            loadCommandModuleSync.failedModules.delete(filePath);

            /* Log da operação */
            if (global?.config?.logIndexer?.value) console.debug(`[CACHE CLEAR] Módulo específico: ${moduleName}`);
            return true;
        }

        /* Se não retornar nada acima, então falhou */
        return false;
    }

    /* Limpa todo o cache */
    commandCache.clear();
    loadCommandModuleSync.failedModules.clear();

    /* Log da operação */
    if (global?.config?.logIndexer?.value) console.debug('[CACHE CLEAR] Todos os módulos');
    return true;
}
...
export default Object.assign(
    (systemName) => controlSystem(systemName),
    {
        ...
        _internal: {
            ...
            resolveCommandPath,
            clearCache,
            getModuleNameFromFilePath,
            getCacheStats: () => ({
                size: commandCache.size,
                hits: commandCache.hits,
                misses: commandCache.misses,
                mappedModules: filePathToModuleMap.size,
                mappedNames: filePathToModuleMap,
            }),
        },
    },
);
