...
function dialParse(
    ...
    try {
        /* Faz a execução */
        const respDial = child.execSync(`bash "lib/Scripts/JSON.sh" "${textDial}" "${dialLang}" "${textType}" "${funcType}" "${dialSize}"`, {
            encoding: 'utf8',
            stdio: [],
        });
...
function getRandomLine(
    ...
    try {
        /* Faz a execução */
        const lineRand = child.execSync(`bash "lib/Scripts/Commands.sh" "line" "${linesGet}" "${fileTexts}"`, {
            encoding: 'utf8',
            stdio: [],
        });
