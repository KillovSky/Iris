...
function isURL(
    ...
    try {
        ...
        if (typeof linkText === 'string') {
			/* Define o RegExp */
			const regExper = createRegExp(envInfo.parameters.linkexp.value);

            /* Verifica se é uma URL */
            envInfo.results.value = {
				isURL: regExper.test(linkText),
				matchedURL: linkText.match(regExper) || ['none'],
			};

			/* Define a URL corretamente, se houver */
			envInfo.results.value.matchedURL = envInfo.results.value.matchedURL[0];
        }
