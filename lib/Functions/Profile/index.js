...
async function getProfileImages(
    ...
    try {
        ...
        if ([kill, env].every((val) => typeof val === 'object' && val !== null)) {
            ...
            const lookupAndPushImage = async (user) => {
                /* Com base na lookup */
                const image = await lookupPhotos(kill, user, groupMembersId);
            ...
            /* 1° foto de perfil */
            const firstImageUser = quotedMsg?.participant || mentionedJidList[0] || Indexer('array').extract(groupMembersId).value;
