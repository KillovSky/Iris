...
function locateFolder(foldername) {
    ...
    const folders = fs.readdirSync(thisDir);
...
function executeSQLite(sqlfolder, typecode, user, chatId, keyup, limit, value) {
    ...
    if ([sqlfolder, typecode, user, chatId, keyup, limit, value].some((s) => s !== null)) {
        ...
        const sqlFilePath = `${thisDir}/${file}/${typecode}.sql`;
...
function getValues(
    ...
    try {
        ...
        const isStringValid = typeof file === 'string' && /@s.whatsapp.net|false|@lid/gi.test(user) && /@g.us|false|@s.whatsapp.net|getall/gi.test(chatId);
...
function updateValues(
    ...
    try {
        ...
        const areValuesValid = (
            ...
            && /@s.whatsapp.net|false|@lid/gi.test(user)

            /* Verifica se o chatId contém uso correto */
            && /@g.us|false|@s.whatsapp.net|@lid/gi.test(chatId)
        );
