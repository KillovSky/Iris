...
/* Imports */
import ffmpeglocation from '@ffmpeg-installer/ffmpeg';
import fluent_ffmpeg from 'fluent-ffmpeg';
import Indexer from '../../index.js';
import extender from './messages.js';

/* Ajusta o ffmpeg para sobrepor o uso dos módulos que usem ele */
global.fluent_ffmpeg_1 = fluent_ffmpeg;
            ...
            kill.ev.process(async (events) => {
                ...
                if (events['messages.upsert']) {
                    ...
                    if (sucessfulInit === false) {
                        ...
                        global.irisNumber = (global.irisNumber === false
                            ? (kill?.user?.id?.includes('@lid') ? `${kill?.user?.id.split('@')[0].split(':')[0]}@lid` : `${kill?.user?.id.split('@')[0].split(':')[0]}@s.whatsapp.net`)
                            : global.irisNumber
                        );
