...
function createListener(kill, saveCreds, genSession, startOptions, indexlaunch) {
    /* Caso a função raiz seja invalida */
    if (
        typeof kill === 'object'
        && [saveCreds, genSession].some((t) => typeof t === 'function')
        && typeof startOptions === 'object' && /[0-9]+/g.test(indexlaunch)
    ) {
        ...
        try {
            kill.ev.process(async (events) => {
                ...
                if (events['connection.update']) {
                    /* Envia pro reload */
                    await Indexer('states').spec(kill, events['connection.update'], genSession, startOptions, indexlaunch);
                }
                ...
                if (events['group-participants.update']) {
                    fs.writeFileSync('./upreceived.json', JSON.stringify(events['group-participants.update'], null, 4));
            ...
            global.irisNumber = `${kill.user.id.split('@')[0].split(':')[0]}@c.us`;
