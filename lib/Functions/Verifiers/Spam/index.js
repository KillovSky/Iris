        ...
        if (typeof kill === 'object' && typeof env === 'object') {
            ...
            if (conditions) {
                ...
                if (functions.antispam.ban === true && isMedia) {
                    ...
                    await kill.sendMessage(chatId, { text: language(region, 'Security', 'Notice', true, true, { userFormated, notice: 'SPAM' }), mentions: groupAdmins });

                    /* Avisa que foi SPAM e limita por 5 minutos */
                } else {
                    /* Avisa do SPAM e bloqueia os comandos da pessoa por x minutos */
                    await kill.sendMessage(chatId, { text: language(region, 'Security', 'Mute', true, true, { userFormated, timeLimit: (config.spammerTime.value / 1000 / 60) }), mentions: [...groupAdmins, user, ...config.owner.value] });
                }
