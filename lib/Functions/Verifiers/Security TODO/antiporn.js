...
function linkURL(URLs) {
    /* Faz a checagem de pornografia em URLs */
    const resultPorn = URLs.some((mURL) => {
        /* Caso já tenha dado true, pula */
        if (confirmBan === true) return true;

        /* Faz a verificação */
        isPorn(mURL, function(error, status) {
            /* Edita o resultado usando nulify */
            confirmBan = (status ?? false);
        });
...
exports.init = async (k, m) => {
	...
	try {
		...
		if (Object.keys(functions.vips[chatId] || []).includes(user) && config.VIP_Links == true) return true;
