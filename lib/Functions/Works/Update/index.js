    ...
    try {
        ...
        if (envInfo.parameters.enableChecker.value) {
            ...
            if (
                ...
                console.log(Indexer('color').echo('[UPDATE]', 'green').value, Indexer('color').echo(Indexer('sql').languages(region, 'Updates', 'Updated', true, true).value, 'yellow').value);

                /* Se elas não forem iguais */
            } else {
                /* Diz sobre uma atualização disponível */
                console.log(Indexer('color').echo('[UPDATE]', 'red').value, Indexer('color').echo(`${Indexer('sql').languages(region, 'Updates', 'Available', true, true).value} → [${packVersion.data.version} ~ ${packVersion.data.build_name.toUpperCase()} ~ ${packVersion.data.build_date.toUpperCase()}] | ${packVersion.data.homepage}`, 'yellow').value);
            }
