...
/* Define o nome de usuário e senha dinâmicos/estáticos */
const userLogin = String(envInfo.parameters.defaultUser.value || Indexer('string').generate(envInfo.parameters.userLogin.value));
const userPass = String(envInfo.parameters.defaultPass.value || Indexer('string').generate(envInfo.parameters.userPass.value));
    ...
    return new Promise((resolve) => {
        ...
        try {
            ...
            if (envInfo.parameters.enableConsole.value === true) {
                ...
                /* Retorna o código digitado no terminal web */
                app.post('/done', (req, res) => {
                    /* Adquire o arquivo HTML */
                    const HTMLData = fs.readFileSync('./lib/Wiki/Pages/Terminal/index.html').toString();

                    /* Define a página a editar */
                    let pageCode = HTMLData;

                    /* Define uma nova console log */
                    console.log = function log(input) {
                        /* Resultado padrão */
                        pageCode = HTMLData.replace('#CONSOLEOUTPUT#', util.format(input));

                        /* Verifica se é uma object correta */
                        if (typeof input === 'object' && input !== null && !(input instanceof Function) && Object.keys(input ?? '').length !== 0) {
                            /* Formata como Object */
                            pageCode = HTMLData.replace('#CONSOLEOUTPUT#', `<code>${JSON.stringify(input, functionString, 4)}</code>`);
                        }

                        /* Se desejar printar o console.log na tela, ative a linha abaixo: */
                        /* defConsole(util.format(input) + '\n'); */
                        return util.format(input);
                    };

                    /* Try-Catch para caso o código seja invalido */
                    try {
                        /* Executa o código e printa ele na HTML/tela */
                        /* eslint-disable-next-line no-eval */
                        const evalData = eval(req.body.codeload);

                        /* Printa no console/HTML */
                    ...
                    /* Envia a página de terminal com output */
                    res.send(Buffer.from(pageCode));

                    /* Restaura a console.log */
                    console.log = print;
                });

                /* Inicia o APP HTTPS */
                const httpsServer = (https.createServer(httpsOptions, app)
                    /* Porta aleatória ou pré-configurada */
                    .listen(envInfo.parameters.httpsPort.value || 0)
                );

                /* Inicia o APP HTTP */
                const httpServer = (http.createServer(httpApp)
                    /* Porta aleatória ou pré-configurada */
                    .listen(envInfo.parameters.httpPort.value || 0)
                );
        ...
        /* Retorna o dialogo */
        resolve(envInfo.results.success);
    });
}
