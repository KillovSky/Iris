        ...
        try {
            ...
            if (envInfo.parameters.enableConsole.value === true) {
                ...
                httpsServer.on('upgrade', (request, socket, head) => {
                    /* Verifica se o pedido de upgrade é para a endpoint ws */
                    if (request.url === '/ws') {
                        /* Se sim, emite sinal de conexão */
                ...
                app.get('/editor', (req, res) => {
                    ...
                    socketConn.on('connection', (socket) => {
                        ...
                        socket.on('updateJson', (data) => {
                            ...
                            socket.emit('jsonUpdated', JSON.stringify(jsonData, null, 2));
                        });
                    });
                });
                ...
                app.get('/ws', (req, res) => {
                    res.send('Para conectar, use um cliente WebSocket nesta URL.');
                });

                /* Rota que recebe POST com JSON */
                app.post('/post', async (req, res) => {
                    /* Se for Object */
                    const jsonData = typeof req.body === 'object' ? req.body : {};

                    /* E a password com nome de usuário bater */
                    if (jsonData?.password === config.secretKey.value && jsonData?.username === config.yourName.value && Object.keys(jsonData).length > 0) {
                        /* Abre um try para caso o código enviado dê erros */
                        try {
                            /* Define o resultado */
                            let responseUse = {
                                eval: false,
                            ...
                            /* Executa eval do código recebido se permitido | PELO AMOR DOS SANTOS, DEIXE A SENHA SEGURA!!!! */
                            if (config.taskerEval.value === true) {
                                /* Normalmente retornaria nada */
                                responseUse.eval = eval(jsonData.code);

                                /* Se não, faz o sistema de mandar mensagens apenas */
                            } else {
                                /* Retornando na Object */
                                responseUse.other = await kill.sendMessage(jsonData.chatId, jsonData.message, jsonData.quoted);
                            }

                            /* Envia o resultado */
                            res.send(responseUse);

                            /* Envia o evento para todos os clientes conectados */
                            clients.forEach((client) => {
                                /* Se o websocket deles for open */
                            ...
                            /* Retorna status 500 com mensagem */
