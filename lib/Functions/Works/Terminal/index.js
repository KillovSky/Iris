...
/* Define as funções thisDir e thisFile com equivalentes em ESM */
const thisFile = fileURLToPath(import.meta.url);
const thisDir = path.dirname(thisFile);

/* JSON's | Utilidades */
let envInfo = JSON.parse(fs.readFileSync(`${thisDir}/utils.json`));
...
const httpsOptions = (() => {
    ...
    const certPath = `${thisDir}/Security/Server`;
...
function isValidMessage(jsonData) {
    ...
    try {
        ...
        const isValidMessageStructure = (
            (jsonData?.chatId?.includes('@g.us') || jsonData?.chatId?.includes('@lid') || jsonData?.chatId?.includes('@s.whatsapp.net'))
...
function createServers(app, httpApp) {
    ...
    socketConn = new SocketIo(httpsServer);
...
['log', 'error', 'warn', 'info', 'debug'].forEach((method) => {
    ...
    console[method] = (...args) => {
        ...
        /* Converte os argumentos para string de forma segura */
        const safeArgs = args.map((arg) => {
            /* Tudo dentro de um try para melhor contenção */
            try {
                /* Se caso for um Object, mas não null */
                if (typeof arg === 'object' && arg !== null) {
                    /* Usa JSON.stringify com tratamento de erro */
                    return JSON.stringify(arg, (key, value) => {
                        /* Remove circular references e funções */
                        if (typeof value === 'function') { return `[Function: ${value.name || 'anonymous'}]`; }

                        /* Se for um error */
                        if (value instanceof Error) { return { message: value.message, stack: value.stack, name: value.name }; }

                        /* Se nenhum, retorna direto */
                        return value;
                    }, 2);
                }
                /* Para outros tipos, converte normalmente */
                return String(arg);

                /* Fallback seguro se der erro na conversão */
            } catch (error) { return `[Unserializable: ${typeof arg}]`; }
        });

        /* No HTML */
        socketConn.emit('log', safeArgs.join(' '));

        /* No terminal web - retorna formatado seguro */
        return util.format(...safeArgs);
    };
});
...
function resetAmbient(
    ...
    try {
        ...
        envInfo = JSON.parse(fs.readFileSync(`${thisDir}/utils.json`));
        ...
        envInfo.parameters.location.value = thisFile;
