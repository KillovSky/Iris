...
function createZIP() {
    ...
    const execBackup = Indexer('shell').bash(`bash -c "find ./lib/Databases \\( -path ./lib/Databases/Builds -prune \\) -o \\( -name '*.json' -o -name 'users.db' \\) | zip '${envInfo.parameters.backupFolder.value}/${dateFormated}.zip' -@"`);
