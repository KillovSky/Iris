...
function createZIP() {
    ...
    /* Comando utilizado para backups */
    envInfo.parameters.Backup_Command = `bash -c "find . -path ./node_modules -prune -o -name '*.json' -print | zip '${envInfo.parameters.backupFolder.value}/${dateFormated}.zip' -@"`;

    /* Executa o comando de backup */
    const execBackup = Indexer('shell').bash(envInfo.parameters.Backup_Command);
    ...
    try {
        ...
        if (readDirect.length > envInfo.parameters.maxBackups.value) {
            ...
            objectBackup.value.forEach((erase) => {
                ...
                Indexer('clear').destroy(`${envInfo.parameters.backupFolder.value}/${filesRemoving[erase]}`, 100);
            });
        }
