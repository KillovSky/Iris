...
async function runEvent(
    ...
    try {
        ...
        if ([kill, events, functions].some((f) => typeof f === 'object') && funTypes.includes(fireType) && typeof eventType === 'string') {
            ...
            usersToMention = usersToMention.filter((s) => s?.includes('@s.whatsapp.net') || s?.includes('@lid'));
            ...
            if (cooldown[events.id][fireType].time < Date.now()) {
                ...
                console.log(
                    ...
                    Indexer('color').echo(Indexer('sql').languages(region, 'Console', eventNomeclt, true, true, {
                        userm: `@${usersToMention.join(', ').replace(/@s.whatsapp.net|@lid/g, '')}`,
                ...
                if (typeof functions[fireType].text !== 'string') {
                    ...
                    const formatedPart = usersToMention.map((s) => s.replace(/@s.whatsapp.net|@lid/g, ''));

                    /* Define a mensagem */
                    /* eslint-disable-next-line newline-per-chained-call */
                    const customMessage = (
                        functions[fireType].text
                        .replace(/{userm}/gi, `@${usersToMention.join(' @').replace(/@s.whatsapp.net|@lid/g, '')}`)
                        ...
                        .replace(/{formatuser}/gi, (events.participants[0] || 'N/A').replace(/@s.whatsapp.net|@lid/g, ''))
    ...
    try {
        ...
        if (kill?.sendMessage && typeof events.participants === 'object') {
            ...
            if (!events.participants.includes(irisNumber)) {
                ...
                /* Whitelist */
                const whiteListed = functions.whitelist.values.map((wl) => [
                    `${wl.replace(/@s\.whatsapp\.net|@lid/gi, '')}@s.whatsapp.net`,
                    `${wl.replace(/@s\.whatsapp\.net|@lid/gi, '')}@lid`,
                ]).flat();

                /* Blacklist */
                let blackListed = functions.blacklist.values.map((bk) => [
                    `${bk.replace(/@s\.whatsapp\.net|@lid/gi, '')}@s.whatsapp.net`,
                    `${bk.replace(/@s\.whatsapp\.net|@lid/gi, '')}@lid`,
                ]).flat();
                blackListed = (blackListed.filter((black) => usersToMention.includes(black)
                    && (functions.whitelist.enable === false
                    || (functions.whitelist.enable === true && !whiteListed.includes(black))))
                );

                /* Antifake */
                let antiFaked = usersToMention.map((fk) => [
                    `${fk.replace(/@s\.whatsapp\.net|@lid/gi, '')}@s.whatsapp.net`,
                    `${fk.replace(/@s\.whatsapp\.net|@lid/gi, '')}@lid`,
                ]).flat();
                antiFaked = (antiFaked.filter((fake) => functions.antifake.values.some((af) => fake.startsWith(af)
                    && !whiteListed.some((wl) => fake.startsWith(wl) && functions.whitelist.enable === true)))
                );
