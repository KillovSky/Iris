        ...
        if ([kill, events, functions].some((f) => typeof f === 'object') && funTypes.includes(fireType) && typeof eventType === 'string') {
            ...
            if (cooldown[events.id][fireType].time < Date.now()) {
                ...
                if (typeof functions[fireType].text !== 'string') {
                    ...
                    const canvaGreetings = await Indexer('cards').greets(profilePicture, eventType, Indexer('sql').languages(region, 'Greets', `${eventTypeName}Title`, true, true, {}).value, Indexer('sql').languages(region, 'Greets', `${eventTypeName}Description`, true, true, {}).value);
                    ...
                    const customMessage = functions[fireType].text.replace(/{userm}/gi, `@${usersToMention.join(' @').replace(/@s.whatsapp.net/g, '')}`).replace(/{groupm}/gi, groupMetadata.subject).replace(/{desc}/gi, groupMetadata.desc).replace(/\\n/gi, '\n').replace(/\{([^}]*)\}/gi, (match, key) => Indexer('others').repl(toReplacer, match, key));
