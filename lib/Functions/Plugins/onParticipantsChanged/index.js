    ...
    try {
        ...
        if (typeof kill?.sendMessage === 'function' && typeof events?.participants === 'object') {
            ...
            if (!events.participants.some((ids) => ids === irisNumber)) {
                ...
                /* Blacklist */
                const blackListed = functions.blacklist.block.filter((black) => usersToMention.includes(black) && (functions.whitelist.enable === false || (functions.whitelist.enable === true && !functions.whitelist.unblock.includes(black))));

                /* Antifake */
                const antiFaked = usersToMention.filter((fake) => functions.antifake.block.some((af) => fake.startsWith(af) && !functions.whitelist.unblock.some((wl) => fake.startsWith(wl) && functions.whitelist.enable === true)));
