...
async function runEvent(
    ...
    cooldown[events.id] = Object.keys(cooldown[events.id] || {}).includes('time') ? cooldown[events.id] : envInfo.parameters.lasttext.value;

    /* Define os tipos de funções */
    const funTypes = Object.keys(envInfo.parameters.timedate.value);
    ...
    try {
        ...
        if ([kill, events, functions].some((f) => typeof f === 'object') && funTypes.includes(fireType)) {
            ...
            if (cooldown[events.id][fireType].time < Date.now()) {
                ...
                cooldown[events.id][fireType].time = Date.now() + (
                    ...
                    Number(envInfo.parameters.timedate[fireType]) * 60000
                );

                /* Determina se roda o padrão ou uma mensagem customizada */
                if (typeof functions[fireType].customText !== 'string') {
                    /* Envia a mensagem customizada */
                    envInfo.results.value = await kill.sendMessage(events.id, {
                        text: language(region, 'Greets', eventNomeclt, true, true, { userm: `@${usersToMention.join(' @').replace(/@s.whatsapp.net/g, '')}`, groupm: groupMetadata.subject }),
                        mentions: usersToMention,
                    });

                    /* Caso a mensagem customizada tenha marcação */
                } else if (functions[fireType].customText.includes('userm') || functions[fireType].customText.includes('groupm')) {
                    /* Envia a mensagem customizada por marcação */
                    envInfo.results.value = await kill.sendMessage(events.id, { text: functions[fireType].customText.replace('{userm}', `@${usersToMention.join(' @').replace(/@s.whatsapp.net/g, '')}`).replace('{groupm}', groupMetadata.subject), mentions: usersToMention });

                    /* Se a mensagem customizada não tem marcação */
                } else {
                    /* Envia a mensagem customizada sem @ */
                    envInfo.results.value = await kill.sendMessage(events.id, {
                        text: functions[fireType].customText,
                    });
                }
                ...
                /* Adiciona os não marcados na lista para a próxima */
    ...
    try {
        ...
        if (typeof kill?.sendMessage === 'function' && typeof events?.participants === 'object') {
            ...
            if (!events.participants.some((ids) => ids === irisNumber)) {
                ...
                const functions = Indexer('sql').get('groups', null, events.chat).value;
                const assignEvent = events;

                /* Define os termos de cada uso */
                /* Blacklist */
                const blackListed = functions.blacklist.values.filter((black) => usersToMention.includes(black) && (functions.whitelist.enable === false || (functions.whitelist.enable === true && !functions.whitelist.values.includes(black))));

                /* Antifake */
                const antiFaked = usersToMention.filter((fake) => functions.antifake.values.some((af) => fake.startsWith(af) && !functions.whitelist.values.some((wl) => fake.startsWith(wl) && functions.whitelist.enable === true)));
                ...
                if (assignEvent.action === 'add' && functions.blacklist.enable === true && blackListed.length > 0) {
                    ...
                    /* Define o resultado */
                    tempInfo.results.value = await runEvent(kill, assignEvent, functions, 'blacklist');
                ...
                if (assignEvent.action === 'add' && functions.antifake.enable === true && antiFaked.length > 0) {
                    ...
                    /* Define o resultado */
                    tempInfo.results.value = await runEvent(kill, assignEvent, functions, 'antifake');
                ...
                if (assignEvent.action === 'add' && functions.welcome.enable === true) {
                    /* Define o resultado */
                    tempInfo.results.value = await runEvent(kill, events, functions, 'welcome');
                ...
                if (assignEvent.action === 'remove' && functions.goodbye.enable === true) {
                    /* Define o resultado */
                    tempInfo.results.value = await runEvent(kill, events, functions, 'goodbye');
