...
async function memeMaker(
    ...
    try {
        ...
        if (typeof typememe === 'string' && Array.isArray(avatars)) {
            ...
            let myAvatares = avatars.map((ulk) => (Indexer('regexp').urls(ulk).value.isURL ? ulk : 'NONE'));
            ...
            if (myAvatares.length !== 0) {
                ...
                memetic = memetic === 'memes' ? 'gay' : memetic;
                memetic = memetic === 'trash' ? 'delete' : memetic;
                memetic = memetic === 'slap' ? 'batslap' : memetic;

                /* Converte a mídia e salva */
