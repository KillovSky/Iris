...
async function unikeMemes(profilePictures, memename) {
    ...
    try {
        ...
        if (Array.isArray(profilePictures) && typeof memename === 'string') {
            ...
            const backgroundImage = await canvas.loadImage(path.normalize(`${__dirname}/images/${memename}.jpg`));
            let createdCanvas = canvas.createCanvas(backgroundImage.width, backgroundImage.height);
            let context = createdCanvas.getContext('2d');
...
async function memeMaker(
    ...
    environment = envInfo.functions.memes.arguments.env.value,
) {
    /* Define um resultado padrão */
    ...
    try {
        ...
        if (typeof typememe === 'string' && Array.isArray(avatars)) {
            ...
            if (!['cmm', 'ohno', 'clyde'].includes(typememe.toLowerCase())) {
            ...
            if (myAvatares.length !== 0) {
                ...
                switch (memetic) {
                    ...
                    case 'clyde':
                        ...
                        /* Cria o meme e define na env */
                        envInfo.results.value = await canvacord.Canvas[memetic](environment.value.body || 'Say something!');
                    break;

                    /* Canvacord, Texto com Imagem */
                    case 'opinion':
                        /* Cria o meme e define na env */
                        envInfo.results.value = await canvacord.Canvas[memetic](myAvatares[0], environment.value.body || 'Say something!');
                    break;
